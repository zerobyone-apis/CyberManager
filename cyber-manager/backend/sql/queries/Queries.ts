...
export default class Queries {
  private queries: Record<string, any> = {
    ...
    [ORDER_TABLE]: {
      ...
      reparacion: `UPDATE ${ORDER_TABLE} SET clientName = $1, article = $2, isCanceled = $3, deliverDate = $4, repairDate = $5, reparation = $6, warranty = $7, price = $8, status = $9, replacementPrice = $10 WHERE id = $11`,
      delete: `DELETE FROM ${ORDER_TABLE} WHERE id = $1`,
      cancel: `UPDATE ${ORDER_TABLE} SET isCanceled = $1 WHERE id = $2`,
      arqueo: `SELECT SUM(price) as totalPrice, SUM(replacementPrice) as totalReplacementPrice, sum(price - replacementPrice) as netoPrice, count(price) as cantArticles from ${ORDER_TABLE} where (deliverDate BETWEEN $1 and $2) and status like '%Entregado%'`
    },
