...
export default class Queries {
  ...
  private queries: any = {
    ...
    event: {
      add:
        "INSERT INTO organize.event(name, location, start, end, description, guestsNumber, created) VALUES(?,?,?,?,?,?,?);",
      ...
      link:
        "INSERT INTO organize.uses(idEvent, idQuestionnaire, idOption) VALUES(?,?,?);"
    },
    ...
    questionnaire: {
      add:
        "INSERT INTO organize.questionnaire(idUser, name, category) VALUES(?,?,?);",
      getId:
        "SELECT id FROM organize.questionnaire q where q.idUser = ? AND q.name = ? and q.category = ?;",
      save:
        "UPDATE organize.questionnaire SET idUser=?, name=?, category=? WHERE id=?;",
      delete: "DELETE FROM organize.questionnaire WHERE id=?;",
      getByIdUser:
        "SELECT id, name, category FROM organize.questionnaire where questionnaire.idUser = ?;",
      getIdByIdQuestionnaire:
        "SELECT idOption FROM organize.questionnaire_option where idQuestionnaire = ?;",
      deleteRelation:
        "DELETE FROM organize.questionnaire-option WHERE idQuestionnaire=?;",
      // can add userId in the future
      getByIdEvent:
        "SELECT DISTINCT q.* from questionnaire q, uses u where u.idEvent=? and q.id=u.idQuestionnaire;"
    },

    // question
    question: {
      add:
        "INSERT INTO organize.question(idType, name, category) VALUES(?,?,?);",
      getByIdType: "SELECT * FROM organize.question q where q.idType = ?;",
      getId:
        "SELECT id FROM organize.question q where q.idType = ? AND q.name = ? AND q.category = ?;",
      save:
        "UPDATE organize.question SET idType=?, name=?, category=? WHERE id=?;",
    ...
    timeLimit: {
      add:
        "INSERT INTO organize.timelimit(idEvent, idQuestion, maxtime) VALUES(?,?,?);",
      setMaxTime:
        "UPDATE organize.timelimit SET maxtime=? WHERE idEvent=? AND idQuestion=?;",
      delete:
        "DELETE FROM organize.timelimit where WHERE idEvent=? AND idQuestion=?;",
    ...
    userSystem: {
      add:
        "insert into organize.usersystem(username, password, email) values(?,?,?);",
      get:
        "SELECT id, username, password FROM organize.usersystem where email=? LIMIT 1;",
      existsEmail: "select email from organize.usersystem where email=?",
      getIdByEmail: "select id from organize.usersystem where email=?",
      save:
        "update organize.usersystem set username=?, password=?, email=? where id=?;",
