...
export default class Queries {
  private queries: Record<string, any> = {
    ...
    [ORDER_TABLE]: {
      ...
      arqueo: `SELECT SUM(price) as Total_Price, SUM(replacementPrice) as Total_Replacement_Price, sum(price - replacementPrice) as Neto_price from ${ORDER_TABLE} where (deliverDate BETWEEN ? and ?) and status like '%Entregado%'`
    },
